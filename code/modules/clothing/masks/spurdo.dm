/obj/item/clothing/mask/spurdo
	...
	icon_state = "spurdo"
	worn_icon_state = "spurdo"
	icon = 'icons/obj/clothing/masks.dmi'
	alternate_screams = list('sound/voice/finnish_1.ogg', 'sound/voice/finnish_2.ogg', 'sound/voice/finnish_3.ogg')
...
/obj/item/clothing/mask/spurdo/handle_speech(datum/source, list/speech_args)
	if(copytext(speech_args[SPEECH_MESSAGE], 1, 2) != "*")
		...
		var/list/spurdo_words = strings("word_replacement.json", "spurdo") // Technically not modular, but unless we want to write an entire function for one damn mask then we'll just have this here.
