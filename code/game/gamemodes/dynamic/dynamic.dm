...
/datum/game_mode/dynamic/pre_setup()
	...
	dlog("Dynamic mode parameters for the round:")
	dlog("Centre is [GLOB.dynamic_curve_centre], Width is [GLOB.dynamic_curve_width], Forced extended is [GLOB.dynamic_forced_extended ? "Enabled" : "Disabled"], No stacking is [GLOB.dynamic_no_stacking ? "Enabled" : "Disabled"].")
	dlog("Stacking limit is [GLOB.dynamic_stacking_limit], Classic secret is [GLOB.dynamic_classic_secret ? "Enabled" : "Disabled"].")
...
/datum/game_mode/dynamic/post_setup(report)
	...
	addtimer(CALLBACK(src, .proc/send_intercept), rand(1 MINUTES, 3 MINUTES))
	..()

/// A simple roundstart proc used when dynamic_forced_roundstart_ruleset has rules in it.
/datum/game_mode/dynamic/proc/rigged_roundstart()
	dlog("[GLOB.dynamic_forced_roundstart_ruleset.len] rulesets being forced. Will now attempt to draft players for them.")
	for (var/datum/dynamic_ruleset/roundstart/rule in GLOB.dynamic_forced_roundstart_ruleset)
		configure_ruleset(rule)
		dlog("Drafting players for forced ruleset [rule.name].")
		rule.mode = src
...
/datum/game_mode/dynamic/proc/picking_specific_rule(ruletype, forced = FALSE)
	...
	if((new_rule.acceptable(GLOB.alive_player_list.len, threat_level) && new_rule.cost <= threat) || forced)
...
/datum/game_mode/dynamic/process()
	...
	if (midround_injection_cooldown < world.time)
		...
		dlog("Checking for midround injection.")
		...
		if (get_injection_chance())
			...
			for (var/datum/dynamic_ruleset/midround/rule in midround_rules)
				if (!rule.weight)
					continue
				if (rule.acceptable(GLOB.alive_player_list.len, threat_level) && threat >= rule.cost)
...
/datum/game_mode/dynamic/proc/dlog(text)
