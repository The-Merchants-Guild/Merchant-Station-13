...
/obj/item/melee/execution_sword/attack(mob/living/target, mob/living/user)
	...
	if(!infidel_head || target.stat == DEAD)
		...
		priority_announce("[user] is preparing to execute [target] at [initial(A.name)] in the name of [execution_faction]!","Message from [execution_faction]!", 'sound/misc/notice1.ogg')
