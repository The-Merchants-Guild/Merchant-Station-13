...
/turf/open/floor/mineral/reagent/proc/reagent_act(atom/A)
	if(reagent_type)
		reagent_type.expose_turf(src, methods=TOUCH, 3)
		if(prob(90))
			if(isliving(A))
				reagent_type.expose_mob(A, methods=TOUCH, 3)
			else if(isturf(A))
				reagent_type.expose_turf(A, methods=TOUCH, 3)
			else if(isobj(A))
				reagent_type.expose_obj(A, methods=TOUCH, 3)
		else if(reagent_type)
			for(var/atom/AM in view(1, src))
				if(isliving(AM))
					reagent_type.expose_mob(AM, methods=TOUCH, 3)
				else if(isturf(AM))
					reagent_type.expose_turf(AM, methods=TOUCH, 3)
				else if(isobj(AM))
					reagent_type.expose_obj(AM, methods=TOUCH, 3)
