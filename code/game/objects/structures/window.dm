	...
	switch(state)
		if(RWINDOW_SECURE)
			if(tool.tool_behaviour == TOOL_SCREWDRIVER)
				user.visible_message(span_notice("[user] begins to turn the screws and remove them..."),
										span_notice("You turn the screws..."))
...
/obj/structure/window/reinforced/examine(mob/user)
	...
	switch(state)
		if(RWINDOW_SECURE)
			. += span_notice("It's secured with <b>screws</b> that need to be removed.")
		if(RWINDOW_BOLTS_OUT)
			. += span_notice("The screws have been removed, revealing a small gap you could fit a <b>prying tool</b> in.")
		if(RWINDOW_POPPED)
			. += span_notice("The main plate of the window has popped out of the frame, exposing some <b>bolts</b> that look like they can be removed.")
