...
/datum/eldritch_knowledge/blade_dance/on_gain(mob/user)
	RegisterSignal(user, COMSIG_HUMAN_CHECK_SHIELDS, .proc/on_shield_reaction)

/datum/eldritch_knowledge/blade_dance/on_lose(mob/user)
	UnregisterSignal(user, COMSIG_HUMAN_CHECK_SHIELDS)

/datum/eldritch_knowledge/blade_dance/proc/on_shield_reaction(
	...
	if(source.incapacitated(FALSE, TRUE))
		return
...
/datum/eldritch_knowledge/mark/blade_mark/create_mark(mob/living/source, mob/living/target)
...
/datum/eldritch_knowledge/mark/blade_mark/trigger_mark(mob/living/source, mob/living/target)
	. = ..()
	if(!.)
		return
...
/datum/eldritch_knowledge/duel_stance/on_gain(mob/user)
...
/datum/eldritch_knowledge/duel_stance/on_lose(mob/user)
...
/datum/eldritch_knowledge/duel_stance/proc/on_wound_gain(mob/living/source, datum/wound/gained_wound, obj/item/bodypart/limb)
	SIGNAL_HANDLER
...
/datum/eldritch_knowledge/final/blade_final/on_finished_recipe(mob/living/user, list/selected_atoms, turf/loc)
	. = ..()
	priority_announce("[generate_eldritch_text()] Master of blades, the Colonel's disciple, [user.real_name] has ascended! Their steel is that which will cut reality in a maelstom of silver! [generate_eldritch_text()]","[generate_eldritch_text()]", ANNOUNCER_SPANOMALIES)
...
/datum/eldritch_knowledge/final/blade_final/proc/on_eldritch_blade(mob/living/source, mob/living/target, obj/item/melee/sickly_blade/blade)
	SIGNAL_HANDLER
