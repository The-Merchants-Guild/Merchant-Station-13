...
/obj/item/badmin_stone/clown
	...
	ability_text = list("HELP INTENT: fire banana cream pies",
		"COMBAT MODE: Spawn the Traps!",
		"DISARMS: Throw an angry monkey that will hunt whatever you target!")
...
/obj/item/badmin_stone/clown/DisarmEvent(atom/target, mob/living/user, proximity_flag)
	...
	var/mob/living/carbon/human/species/monkey/monkey = new(monkey_loc)
	if(isliving(target))
...
/obj/effect/proc_holder/spell/self/infinity/party_popper/cast(list/targets, mob/living/carbon/user)
...
/obj/effect/proc_holder/spell/self/infinity/honksong/proc/dance_setup(mob/living/user)
	...
	FOR_DVIEW(var/turf/t, 3, get_turf(user),INVISIBILITY_LIGHTING)
		if(t.x == cen.x && t.y > cen.y)
			var/obj/item/flashlight/spotlight/staticlight/L = new /obj/item/flashlight/spotlight/staticlight(t)
			L.light_color = LIGHT_COLOR_BLOOD_MAGIC
...
/obj/effect/proc_holder/spell/self/infinity/honksong/proc/lights_spin(mob/living/user)
	for(var/obj/item/flashlight/spotlight/staticlight/glow in spotlights) // The multiples reflects custom adjustments to each colors after dozens of tests
...
/obj/effect/proc_holder/spell/self/infinity/cake/proc/GetMostEatenCake()
	var/obj/item/food/cake/birthday/infinity/nom_nom
	for(var/obj/item/food/cake/birthday/infinity/cake in cakes)
		if(!QDELETED(cake) && (!nom_nom || cake.cakest.bitecount > nom_nom.cakest.bitecount))
			nom_nom = cake
	return nom_nom

/obj/effect/proc_holder/spell/self/infinity/cake/cast(list/targets, mob/user)
	if(CountCakes() >= 2)
		var/obj/item/food/cake/birthday/infinity/nom_nom = GetMostEatenCake()
		if(istype(nom_nom))
	...
	cakes += new /obj/item/food/cake/birthday/infinity(get_turf(user))

/obj/item/food/cake/birthday/infinity
	name = "infinity cake"
	throwforce = 35
	food_reagents = list(/datum/reagent/consumable/nutriment = 20, /datum/reagent/consumable/sprinkles = 10, /datum/reagent/consumable/nutriment/vitamin = 5, /datum/reagent/medicine/omnizine = 25)
	tastes = list("cake" = 3, "power" = 2, "sweetness" = 1)
	var/datum/component/edible/cakest



/obj/item/food/cake/birthday/infinity/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	. = ..()
...
/obj/vehicle/sealed/car/thanos/relaymove(mob/user, direction)
	. = ..()
	if(. && bloodiness)
...
/mob/living/carbon/human/Cross(atom/movable/AM)
