...
/obj/item/clothing/glasses/nano_goggles/proc/nvgmode(mob/user, forced = FALSE)
	var/mob/living/carbon/human/H = user
	if(H.glasses != src)
		return
	if(!ishuman(user))
...
/obj/item/clothing/suit/space/hardsuit/nano/proc/cut_nano_energy(amount, delay = 0)
	...
	if(!shutdown && (forced || (cell?.charge && mode != suitmode)))
		...
		switch(suitmode)
			if(NANO_ARMOR)
				...
				Wearer.remove_movespeed_modifier(/datum/movespeed_modifier/nanosuit)
			...
			if(NANO_SPEED)
				...
				Wearer.add_movespeed_modifier(/datum/movespeed_modifier/nanosuit)
...
/obj/item/clothing/suit/space/hardsuit/nano/equipped(mob/user, slot)
	...
	if(slot == ITEM_SLOT_OCLOTHING)
		...
		if(is_station_level(T.z))
			priority_announce("[user] has engaged [src] at [get_area(src)]!","Message from The Syndicate!", sound = 'sound/misc/nanosuitengage.ogg')
...
/datum/martial_art/nanosuit/proc/HeadStomp(mob/living/carbon/human/A, mob/living/carbon/human/D)
	...
	if(head)
		...
		playsound(get_turf(A), 'sound/misc/squishy.ogg', 75, TRUE, -1)
...
/datum/martial_art/nanosuit/harm_act(mob/living/carbon/human/A, mob/living/carbon/D)
	var/picked_hit_type = pick("punches", "kicks")
	var/bonus_damage = 10
	var/quick = FALSE
	if(D.resting || D.body_position == LYING_DOWN)//we can hit ourselves
		bonus_damage += 5
		picked_hit_type = "stomps on"
		if(A.zone_selected == BODY_ZONE_HEAD && D.get_bodypart(BODY_ZONE_HEAD) && (!A.resting || !A.body_position == LYING_DOWN))
	...
	if(D != A && !D.stat && (!D.IsParalyzed() || !D.IsStun())) //and we can't knock ourselves the fuck out/down!
		...
		else if(A.resting && D.body_position != LYING_DOWN) //but we can't legsweep ourselves!
	...
	if(!A.resting || !A.body_position == LYING_DOWN)
		if(prob(30))
			quick = TRUE
...
/obj/structure/destructible/clockwork/attack_nanosuit(mob/living/carbon/human/user)
	/*if(is_servant_of_ratvar(user) && immune_to_servant_attacks)
		return FALSE don't worry about this haha*/
	return ..()
...
/datum/martial_art/nanosuit/proc/on_attack_hand(mob/living/carbon/human/owner, atom/target, proximity)
	...
	if(iscarbon(target) && !ishuman(target) && style?.harm_act(owner, target))
		return
	if(owner.combat_mode && !iscarbon(target))
...
/mob/living/carbon/human/proc/Nanosuit_help()
	...
	<span class='notice'>MMA master</span>: Combat mode attacks deals more damage, occasionally trigger series of fast hits and you can leg sweep while lying down.\n\
	<span class='notice'>Highschool bully</span>: Grab someone and attack them with combat mode on to deliver a deadly knock down punch.\n\
	<span class='notice'>Knockout master</span>: Tighten your grip and attack them with combat mode to deliver a very deadly knockout punch.\n\
