...
/client/New()
	. = ..()
	spawn if(src)
		var/age = CONFIG_GET(number/accountage)
		if((src.account_age < age) && age)
			...
			to_chat(src, span_userdanger("Your account is too new to join this server. Come back in [age - src.account_age] days!"))
			QDEL_IN(src, 1)
