...
/obj/machinery/ore_drill/ComponentInitialize()
	AddComponent(/datum/component/extensible_machine, list(
		/obj/machinery/power/ore_drill_power_module = list(image(icon = 'icons/obj/drilling.dmi', icon_state = "power-icon"), 1),
		/obj/machinery/ore_drill_output_module = list(image(icon = 'icons/obj/drilling.dmi', icon_state = "output-icon"), 1)
...
/obj/machinery/ore_drill_box/crowbar_act(mob/living/user, obj/item/tool)
	if (!istype(loc, /turf/open/floor/plating/asteroid/basalt/lava_land_surface/ore))
		to_chat(user, "<span class='warning'>Can't deploy the [src] here!</span>")
		return
	...
	if (do_after(user, 5 SECONDS, src))
		new /obj/machinery/ore_drill(get_turf(src))
		qdel(src)
		return TRUE
